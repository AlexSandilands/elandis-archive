---
name: codex
description: Use this skill when the user asks to "/codex", "create a character called X", "add a minor character", "create a location / point of interest / region called X", "create an item / artifact / magic item called X", "make a new character, location, or item entry", "add a new codex entry", or wants to create, write, or generate any new character, location, or item page for the Elandis wiki. ALSO trigger when the user mentions a character, place (including a forest, gulf, mountain range, or plane), or notable item/artifact that doesn't have a Codex page yet and asks to document, write up, or turn a stub into a proper entry. ALSO trigger when the user says "migrate [name]", "convert [name] to codex", or wants to move a root-level stub into the Codex folder. Supports Characters, Locations (Points of Interest and Regions), and Items — other categories (cities, factions, etc.) will be added later. This skill researches the entry in the synopses, presents findings, gathers the DM's notes, and produces a complete, publication-ready Codex entry. When a restricted companion document is needed, it creates both files.
---
...
**Currently supported:** Characters, Items, and Locations of subtype **Point of Interest** and **Region**. The Location reference also covers the City subtype's routing, but it isn't wired in yet — if the user wants a City, the reference explains how to redirect them. If the user asks for any other category (faction, lore, deity, etc.), tell them that category isn't supported yet and ask if they'd like a character, item, point-of-interest, or region entry instead, or skip the skill.
