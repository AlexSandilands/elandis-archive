...
[SUBJECT]: <the faction's central device — the creature or object and its colours — carved in shallow relief into a square slab of <stone or material> that FILLS the entire frame edge to edge as a flat emblem (a symbol, not an object in a room); <accent metal or leaf> picks it out against the surface, with any border motif chiselled around the edge>
...
[FORMAT]: 1:1 square, the carved surface fills the frame as a centered emblem
```

Derive the device from the faction's identity — a raven for the Order of Ravens, a gryphon for the Green Gryphons. If the faction's lore names a specific symbol (the Order's hidden ravens), use it, and consider repeating it small around the border. Make the device specific — a good prompt conjures one exact crest, not a generic fantasy badge. The `[LIGHTING]` line stays flat and even for every faction; only `[SUBJECT]` and `[MOOD]` change per faction.
