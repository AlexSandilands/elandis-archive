---
Type: Faction
Category: Local Chapter
Parent Faction: "[[Order of Ravens]]"
City: "[[Val Miriel]]"
Leader: "[[Thaddeus Karr]]"
Base: "[[The Gilded Crow]]"
Importance: Major
Status: Active
NoteIcon: faction
cssclasses:
  - hide-header-underline-3
tags:
  - faction
  - faction/chapter
aliases:
  - Val Miriel Nest
  - The Raven's Nest of Val Miriel
---
> [!infobox|wikipedia]
> # Order of Ravens — Val Miriel
> [![[Codex/Assets/Factions/Order_of_Ravens_small.webp|cover hsmall]]](Codex/Assets/Factions/Order_of_Ravens.webp)
> ###### Chapter Information
> Attribute |  Details |
> ---|---|
> Type | Local Chapter |
> Parent Faction | [[Order of Ravens]] |
> City | [[Val Miriel]] |
> Leader | [[Thaddeus Karr\|Thaddeus "Ravenwing" Karr]] |
> Base | [[The Gilded Crow]] |
> ###### Status
> Attribute |  Details |
> ---|---|
> Status | Active |

The **Val Miriel Nest** is the [[Order of Ravens]]' chapter in [[Val Miriel]] — a cell of thieves and spies run out of a tavern cellar in the city's artisan quarter and led by the Nest Father [[Thaddeus Karr\|Thaddeus "Ravenwing" Karr]]. It is among the Order's more active presences, and the heist that defines it helped tip the city into open unrest.

## Overview & Base

The Nest is headquartered beneath [[The Gilded Crow]], a tavern on the Artisan Way, with its entrance concealed behind a keg in the cellar; past it, the chapter is built directly into the [[Underrun]], the warren of passages beneath Val Miriel. Entry is granted to those who give the passphrase — *"I'll have one of your sharpest drinks"* — spoken to the right ear at the bar above.

From this hidden seat Thaddeus "Ravenwing" Karr directs the Order's work in the city: gathering intelligence on the Imperial presence, sheltering operatives passing through, and running the jobs handed down from above. The chapter keeps the tight, compartmentalised discipline the Order is built on, and the Nest Father expects word sent ahead from other Nests to be honoured when contacts arrive.

A side chamber of the Nest holds the Order's own door onto the [[Underrun]] proper — a section of wall that slides back to open into the tunnels beyond, with a pair of eyeholes set into it so a Raven can check the passage is clear before stepping through. It gives the Nest a quiet way in and out beneath the city, away from the cellar stair, and it is the route by which Ravens guide trusted contacts out of Val Miriel entirely.

## Notable Members

| Member | Role | Notes |
|---|---|---|
| [[Thaddeus Karr\|Thaddeus "Ravenwing" Karr]] | Nest Father | Leads the Val Miriel Nest from beneath the Gilded Crow |
| Trak "Shadow" Aeris | Operative | A working Raven of the Nest |
| Gideon "Whisper" Blackthorn | Operative | A working Raven of the Nest |
| [[Lia]] | Fledgling | Joined the Order after Session 17; went rogue during the Bastion heist |

## Local Operations

- **The Bastion heist.** The Nest broke into [[The Bastion]] and stole sensitive Imperial documents. The job went clean until the Fledgling Lia abandoned the plan to attempt the assassination of [[General Varan Brado]] without sanction — an act that helped ignite the Val Miriel riots and left her fate an unresolved problem for Thaddeus to manage.

## [[The Bloody Nails|Campaign: The Bloody Nails]]

#### [[Session 23 - Ruby Falls Goldmine]]

Word reached the party that Thaddeus "Ravenwing" Karr and the Val Miriel Nest expected them, with a further contact waiting at the Val Aran Nest down the road.

#### [[Session 25 - The Chimera]]

The Nest set the Bastion heist in motion, drawing the party into the theft of Imperial documents from within the city.

#### [[Session 27 - The Wanted Dwarf]]

The heist unravelled when Lia broke from the plan to make her attempt on General Varan Brado, leaving Thaddeus to contain the fallout as the city began to riot.

#### [[Session 28 - The Underrun]]

The party moved through the Underrun beneath Val Miriel, the warren in which the Nest makes its home.
